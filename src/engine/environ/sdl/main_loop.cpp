...
namespace engine::sdl
{
	...
	{
		_impl->running = true;
		prevTick = std::chrono::steady_clock::now();
		#ifdef EMSCRIPTEN
		emscripten_set_main_loop_arg(&MainLoop::tickOuter, (void *) this, 0, true);
		#else
		while (_impl->running) {
			tick();
		}
	...
	void MainLoop::tickOuter(void *arg)
	{
		static_cast<MainLoop*>(arg)->tick();
	}
